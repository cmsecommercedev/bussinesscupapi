...
namespace BussinessCupApi.Controllers.Api // Namespace'i kontrol edin
{
    ...
    public class FavouritesController : ControllerBase
    {
        ...
        public async Task<IActionResult> AddTeamToFav([FromQuery] int teamId, [FromQuery] string userToken,string MacID)
        {
        ...
        public async Task<IActionResult> AddUserToAll([FromQuery] string userToken)
        {
            if (string.IsNullOrWhiteSpace(userToken))
                return BadRequest("Geçersiz macid veya kullanıcı token bilgisi.");


            var result = await _notificationManager.SubscribeToTopicAsync(new[] { userToken }, "all_users");
