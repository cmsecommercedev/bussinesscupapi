...
namespace BussinessCupApi.Controllers
{
    ...
    public class CityAdminController : Controller
    {
        ...
        private readonly ILogger<CityAdminController> _logger;
        ...
        public CityAdminController(ApplicationDbContext context, IConfiguration configuration, ILogger<CityAdminController> logger,
            LeagueManager leagueManager, CloudflareR2Manager r2Manager,
            NotificationManager notificationManager, OpenAiManager openAiManager, CustomUserManager customUserManager) // LeagueManager constructor'a eklendi
        {
            _context = context;
            ...
            _openAIManager = openAiManager; // OpenAiManager eklendi
        ...
        public async Task<IActionResult> Dashboard()
        {
            try
            {
                ...
                int _cityID = Convert.ToInt32(user.CityID); // ApplicationUser sınıfında varsa 

                var leagues = await _context.Leagues
                   .Where(l => l.CityID == _cityID)
                    .Select(l => new
                    {
                        l.LeagueID,
                        l.Name,
        ...
        public async Task<IActionResult> EditPlayer(int playerId)
        {
            try
            {
                ...
                var currentuser = await _customUserManager.GetUserAsync(User); // Bu, ApplicationUser döner
                int _cityID = Convert.ToInt32(currentuser.CityID); // ApplicationUser sınıfında varsa 


                var teams = await _context.Teams
                    .Where(x => x.CityID == _cityID)
                ...
                var viewModel = new PlayerViewModel
                {
                    ...
                    TeamID = player.TeamID // Player modelinde TeamID nullable ise kontrol ekle
        ...
        public async Task<IActionResult> Teams()
        {
            try
            {
                var user = await _customUserManager.GetUserAsync(User); // ApplicationUser döner
                int _cityID = Convert.ToInt32(user.CityID); // ApplicationUser sınıfında varsa 
                ...
                var teams = await _context.Teams
                    .Join(_context.City, // City tablosuyla join
                          ...
                          (team, city) => new { team, city }) // geçici anonim tip
                    .Where(tc => tc.team.CityID == _cityID) // sadece kullanıcının şehrine ait takımlar
                    .Select(tc => new TeamListViewModel
                    {
                        TeamID = tc.team.TeamID,
                        Name = tc.team.Name,
                        CityName = tc.city.Name
                    })
                    .OrderBy(t => t.Name)
                    .ToListAsync();
        ...
        public async Task<IActionResult> SaveWeekBestTeam([FromBody] RequestWeekBestTeamViewModel model)
        {
            ...
            try
            {
                ...
                else
                {
                    ...
                    return Ok(new { success = true, message = "Haftanın 11'i başarıyla kaydedildi." });
                }
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Haftanın 11'i kaydedilirken/güncellenirken hata oluştu. WeekID: {WeekID}", model.WeekID);
        ...
        public async Task<IActionResult> UpdateMatchStatus([FromBody] UpdateMatchStatusRequest model)
        {
            ...
            try
            {
                ...
                {
                    bool _newExist = _context.MatchNews.Any(MatchNews => MatchNews.MatchID == match.MatchID);

                    if (!_newExist)
                    {
                        // Golleri ve kartları çek
                        var goals = await _context.Goals
                            .Where(g => g.MatchID == match.MatchID)
                            .Include(g => g.Player)
                            .Include(g => g.AssistPlayer)
                            .ToListAsync();

                        var cards = await _context.Cards
                            .Where(c => c.MatchID == match.MatchID)
                            .Include(c => c.Player)
                            .ToListAsync();

                        // Takım adları ve skorlar
                        var homeTeam = (await _context.Teams.FindAsync(match.HomeTeamID))?.Name ?? "Ev Sahibi";
                        var awayTeam = (await _context.Teams.FindAsync(match.AwayTeamID))?.Name ?? "Deplasman";

                        int homeScore = match.HomeScore ?? 0;
                        int awayScore = match.AwayScore ?? 0;

                        // Goller için özet string hazırla
                        string goalDetails = "";
                        foreach (var goal in goals.OrderBy(g => g.Minute))
                        {
                            var scorer = $"{goal.Player?.FirstName} {goal.Player?.LastName}".Trim();
                            var assist = goal.AssistPlayer != null ? $"(Asist: {goal.AssistPlayer.FirstName} {goal.AssistPlayer.LastName})" : "";
                            var type = goal.IsOwnGoal ? " (Kendi Kalesine)" : goal.IsPenalty ? " (Penaltı)" : "";
                            goalDetails += $"{goal.Minute}. dk: {scorer} {assist}{type}\n";
                        }
                        if (string.IsNullOrWhiteSpace(goalDetails)) goalDetails = "Gol yok.\n";

                        // Kartlar için özet string hazırla
                        string cardDetails = "";
                        foreach (var card in cards.OrderBy(c => c.Minute))
                        {
                            var player = $"{card.Player?.FirstName} {card.Player?.LastName}".Trim();
                            var cardType = card.CardType == CardType.Yellow ? "Sarı Kart" : "Kırmızı Kart";
                            cardDetails += $"{card.Minute}. dk: {player} - {cardType}\n";
                        }
                        if (string.IsNullOrWhiteSpace(cardDetails)) cardDetails = "Kart yok.\n";

                        try
                        {
                            var news = await _openAIManager.GenerateMatchSummaryAsync(homeTeam, awayTeam, homeScore, awayScore, goalDetails, cardDetails);

                            // Gerekli bilgileri alıyoruz
                            var homeTeamName = match.HomeTeam.Name;
                            var awayTeamName = match.AwayTeam.Name;
                            var score = $"{match.HomeScore ?? 0} - {match.AwayScore ?? 0}";
                            var matchDateTime = match.MatchDate.ToString("dd MMMM dddd - HH:mm", new System.Globalization.CultureInfo("tr-TR"));

                            // YENİ BAŞLIK: İsteğiniz üzerine başlığı formatlıyoruz.
                            string title = $"Maç Sonu: {homeTeamName} {score} {awayTeamName}";

                            // YENİ ALT BAŞLIK: OpenAI metninin ilk satırını alt başlık yapalım.
                            // Bu, haberin en ilgi çekici kısmını öne çıkarır.
                            string subtitle = news.Split(new[] { '\n' }, StringSplitOptions.RemoveEmptyEntries).FirstOrDefault() ?? "";

                            // MatchNews nesnesi oluştur
                            var matchNews = new MatchNews
                            {
                                Title = title,
                                Subtitle = subtitle,
                                DetailsTitle = title, // Detay başlığını da ana başlıkla aynı yapıyoruz. 
                                Details = news,
                                ...
                                MatchNewsMainPhoto = "https://minifutbolturkiye-test.agx-labs.com/images/matchcardresult.jpg"
                            };

                            _context.Set<MatchNews>().Add(matchNews);

                            await _context.SaveChangesAsync();

                            _logger.LogInformation($"Oluşturulan maç özeti MatchNews tablosuna kaydedildi. NewsId: {matchNews.Id}");
                        }
                        catch (Exception ex)
                        {
                            _logger.LogError(ex, "OpenAI ile maç özeti oluşturulurken veya MatchNews kaydedilirken hata oluştu.");
                        }
        ...
        public class DeleteWeekRequest
        {
            public int WeekId { get; set; }
        }
        [HttpPost]
        public async Task<IActionResult> DeleteWeek([FromBody]DeleteWeekRequest request)
        {
            var week = await _context.Weeks
                .Include(w => w.Matches)
                .FirstOrDefaultAsync(w => w.WeekID == request.WeekId);

            if (week == null)
                return Json(new { success = false, message = "Hafta bulunamadı." });

            if (week.Matches.Any())
                return Json(new { success = false, message = "Bu haftada maç olduğu için silinemez." });

            _context.Weeks.Remove(week);
            await _context.SaveChangesAsync();
            return Json(new { success = true, message = "Hafta başarıyla silindi." });
        }
        ...
        {
            var season = await _context.Season.FirstOrDefaultAsync(s => s.SeasonID == request.SeasonId);

            if (season == null)
                return Json(new { success = false, message = "Sezon bulunamadı." });

            // İlgili haftaları SeasonID ile çek
            var weeks = await _context.Weeks
                .Where(w => w.SeasonID == request.SeasonId)
                .Include(w => w.Matches)
                .ToListAsync();

            if (weeks.Any(w => w.Matches.Any()))
                return Json(new { success = false, message = "Bu sezonda maç olduğu için silinemez." });

            // Haftaları sil
            _context.Weeks.RemoveRange(weeks);
            _context.Season.Remove(season);
            await _context.SaveChangesAsync();
            return Json(new { success = true, message = "Sezon başarıyla silindi." });
        }
        [HttpPost]
